...
<div class="form-group">
    <label style="display:block">Foto Sentra Koperasi RW</label>
    <table class="table">
        <thead class="thead-light">
            <tr>
...
<div class="form-group">
    <label style="display:block">Daftar Koperasi RW</label>
    <a href="/admin/add-koperasirw" class="btn btn-primary mb-3">Tambah Koperasi</a>
    <table class="table" id="tableFoto">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$daf->jenis}}</td>
                <td>
                    <form action="/admin/edit-koperasirw/{{$daf->id}}" method="POST">
                        <a href="/admin/edit-koperasirw/{{$daf->id}}" class="btn btn-info">Edit</a>
                        @csrf
                        @method('DELETE')
                        <input type="submit" class="btn btn-danger my-1" onclick="return confirm('Yakin Ingin Menghapus Koperasi?')" value="Delete">
                    </form>
                </td>
            </tr>
            @empty
